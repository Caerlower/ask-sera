...
/**
 * Collect Groq keys from env.
 * Prefer `GROQ_API_KEYS=key1,key2,key3` — also accepts GROQ_API_KEY, GROQ_API_KEY_2, …
 */
function getGroqApiKeys(): string[] {
  ...
  if (process.env.GROQ_API_KEY?.trim()) numbered.push(process.env.GROQ_API_KEY.trim());
  for (let i = 2; i <= 10; i++) {
 ...
 * (and similar) — only before any successful tokens are returned to the client.
 */
export async function streamChatWithFailover(args: ChatStreamArgs): Promise<Response> {
  ...
  for (let attempt = 0; attempt < candidates.length; attempt++) {
    ...
    try {
      ...
      if (!done && value) {
        ...
        if (streamErr && isFailoverError(streamErr)) {
          ...
          if (attempt < candidates.length - 1) {
            console.warn(
              `[ask-sera] Groq key #${index + 1} rate-limited/unavailable — trying next key`,
            );
            continue;
          }
        }
      ...
      if (isFailoverError(error) && attempt < candidates.length - 1) {
        ...
        console.warn(
          `[ask-sera] Groq key #${index + 1} failed (${isRateLimitError(error) ? "rate limit" : "upstream"}) — trying next key`,
        );
        continue;
      }
      break;
  ...
  console.error("[ask-sera/chat] all keys exhausted", lastError);
  return Response.json({ error: publicErrorMessage(lastError) }, { status: 429 });
}
